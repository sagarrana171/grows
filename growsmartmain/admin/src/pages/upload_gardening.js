...
function Gardening() {
    ...
    var type = 3;
    const [G_title, setname] = useState('');
    const [G_description, setdescription] = useState('');
    const [G_extradescription, setextradescription] = useState('');
    const [G_subtitle, setsubname] = useState('');
    const [G_subdescription, setsubdescription] = useState('');
    const [G_file, setfilename] = useState("");

    const postData = () => {
        var data = new FormData();
        data.append('G_image', G_file);
        data.append('G_title', G_title);
        data.append('G_description', G_description);
        data.append('G_extradescription', G_extradescription);
        data.append('G_subtitle', G_subtitle);
        data.append('G_subdescription', G_subdescription);
        ...
        Axios.post('http://localhost:1137/api/upload_gardening', data, config).then((response) => {
            if (response.data.message) {
                alert(response.data.message);
                window.location = "/Manage_gardening";
            }
        });
    }
                                            ...
                                            <div class="row align-items-center">
                                                <div class="col-md-12">
                                                    ...
                                                    <div class="page-header-title">
                                                        <h5 class="m-b-10">Upload Gardening</h5>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div class="card">
                                                <div class="card-body">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-6">
                                                            <form>
                                                                <a><img src="./resources/assets/images/gardening.jpg" alt="" class=" pic" /></a>
                                                            </form>
                                                        </div>
                                                    </div>
                                                </div>
